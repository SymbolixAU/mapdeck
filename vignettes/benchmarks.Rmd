...
In all cases, plotting raw geojson is the quickets way to get your data to javascript as it doesn't go through any pre-processing. 

However, if you specify one of the `properties` as one of the colour variables, it then has to get converted to `sf`, processed, and back again to GeoJSON. This incurs a fairly big penalty. 
