...
```{r packages, eval = TRUE, message = FALSE, echo = FALSE}
library(mapdeck)
```
...
**However**, if two of your layers of the same type have the same `layer_id`, then `deck.gl` will get confused and will only show the last layer drawn. 

For example, if you have to scatterplot layers, both with the same `layer_id`, only the second one will be shown. 
...
![Arcs](./img/articles/animated_arcs.mp4){width="80%"}
...
mapdeck( token = key, style = mapdeck_style('dark')) %>%
	add_geohash(data = mapdeck::geohash
							, geohash = "geohash"
							, fill_colour = "value"
							, elevation = "value"
...
![Scatter](./img/articles/geohash.png)
