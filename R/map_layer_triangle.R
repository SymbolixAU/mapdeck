mapdeckTriangleDependency <- function() {
	list(
		createHtmlDependency(
			name = "triangle",
			version = "1.0.0",
			src = system.file("htmlwidgets/lib/triangle", package = "mapdeck"),
			script = c("triangle.js"),
			all_files = FALSE
		)
	)
}
...
#' Add Triangle
#'
#' The Triangle Layer renders filled and/or stroked polygons.
#'
...
add_triangle <- function(
	...
	layer_id <- layerId(layer_id, "triangle")

	map <- addDependency(map, mapdeckTriangleDependency())
	...
	js_transitions <- resolve_transitions( transitions, "triangle" )
...
#' @rdname clear
#' @export
clear_triangle <- function( map, layer_id = NULL) {
	layer_id <- layerId(layer_id, "triangle")
	invoke_method(map, "md_layer_clear", map_type( map ), layer_id, "triangle" )
}
